...
int main(){
    ...
    CUDA_CHECK(cudaMalloc((void**)&kk, 3*sizeof(int)));
    ...
    CUDA_CHECK(cudaMemset(kk, 0, 3*sizeof(int)));
    ...
    int ccheck=1; int check=0; int diff=0; int check2=-1;
    int num_blocks = ((num*num)+BLOCK_SIZE-1) / BLOCK_SIZE;
    int num_blocks1 = (num+BLOCK_SIZE-1) / BLOCK_SIZE;
    int* d_in1=&d_in[num]; int* d_out1=&d_out[num];

    /* *kk[0]=diff, kk[1]=check, ccheck= kk[2]*/
    while(diff==0){
        CUDA_CHECK(cudaMemcpy(&check, &kk[1], sizeof(int), cudaMemcpyDeviceToHost)); 
        CUDA_CHECK(cudaMemset(&kk[1],0, sizeof(int)));
        
        //CUDA_CHECK(cudaMemset(&kk[1],0, sizeof(int)));
        //if(check==check2) diff=-1;
        //if(diff!=0) break;
        matmul<<<num_blocks,BLOCK_SIZE>>>(d_in,d_out, d_edges, d_dest, num, ccheck);
        matmul<<<num_blocks,BLOCK_SIZE>>>(d_in1,d_out1, d_edges, d_dest, num, ccheck);
        cudaDeviceSynchronize();

        filter<<<num_blocks1,BLOCK_SIZE>>>(d_in,d_out, num);
        filter<<<num_blocks1,BLOCK_SIZE>>>(d_in1,d_out1, num);
        cudaDeviceSynchronize();
        checking<<<num_blocks1,BLOCK_SIZE>>>(d_in,d_in1,kk,num);
        cudaDeviceSynchronize();
        ccheck++;
        CUDA_CHECK(cudaMemcpy(&diff, &kk[0], sizeof(int), cudaMemcpyDeviceToHost)); 
        CUDA_CHECK(cudaMemcpy(&check2, &kk[1], sizeof(int), cudaMemcpyDeviceToHost));
        if(check==check2) diff=-1; 
    }
...
// }
    ...
    while (list1.empty()==0) {
        ...
        h_in[i+num]=-1;
        list1.pop_front(); 
        //if(source==0&&target==1){printf("%d ",i);}
    } 

    list1.clear();
    //if(source==0&&target==1) printf(" [%d]\n",count);
    }
