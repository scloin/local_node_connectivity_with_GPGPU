...
add POOL
merge memcpy
bfsmain으로 loop end check를 kernel로
*/
...
__global__ void BFSmain(pool P,int * d_c_frontier, int* d_p_frontier);
...
int main(){
    ...
    CUDA_CHECK(cudaMalloc((void**)&devmem, (elen+dlen+6*P.numVertex+2)*sizeof(int)));
    ...
    P.d_frontier=&(devmem[dlen+elen+4*P.numVertex]);
    P.d_c_frontier_tail=&(devmem[dlen+elen+6*P.numVertex+1]);
    P.d_p_frontier_tail=&(devmem[dlen+elen+6*P.numVertex]);
...
void BFS_host(pool P)
{
    int* h_p_frontier_tail=&P.h_visited[-1];
    *h_p_frontier_tail = 2;
    ...
    while (*h_p_frontier_tail > 0&&check==0) { 
        ...
        //CUDA_CHECK(cudaMemcpy(h_p_frontier_tail, P.d_p_frontier_tail, sizeof(int), cudaMemcpyDeviceToHost));
    ...
    BFSmain<<<1,1>>>(P,d_c_frontier,d_p_frontier);
    CUDA_CHECK(cudaMemcpy(P.h_label, P.d_label, 2*P.numVertex*sizeof(int), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(&P.h_returned[0], &P.d_p_frontier_tail[0], sizeof(int), cudaMemcpyDeviceToHost)); 
...
__global__ void BFSmain(pool P,int * d_c_frontier, int* d_p_frontier){
    int check=0;
    while (*P.d_p_frontier_tail > 0&&check==0) { 

        int num_blocks = (*P.d_p_frontier_tail+BLOCK_SIZE-1) / BLOCK_SIZE;
        BFS_Bqueue_kernel<<<num_blocks, BLOCK_SIZE>>>(d_p_frontier, P.d_p_frontier_tail,d_c_frontier, P.d_c_frontier_tail, P.d_edges, P.d_dest, P.d_label, P.d_visited);
        //CUDA_CHECK(cudaMemcpy(h_p_frontier_tail, P.d_p_frontier_tail, sizeof(int), cudaMemcpyDeviceToHost));
        //CUDA_CHECK(cudaMemcpyAsync(P.d_p_frontier_tail, P.d_c_frontier_tail, sizeof(int), cudaMemcpyDeviceToDevice,P.stream1));
        //CUDA_CHECK(cudaMemcpy(P.h_visited, P.d_visited, 2*P.numVertex+1*sizeof(int), cudaMemcpyDeviceToHost));
        ...
        for(int k=0;k<P.numVertex;k++){
            if(P.d_visited[2*k]==1&&P.d_visited[2*k+1]==1&&k!=P.source&&k!=P.target){
                check=1;
                break;
            }
        } 
    ...
    for(int k=0;k<P.numVertex;k++){// h_label[2*k+1], h_visited[2*k], h_visited[2*k+1]);
        if(P.d_visited[2*k]==1&&P.d_visited[2*k+1]==1&&k!=P.source&&k!=P.target){
            if(min>P.d_label[2*k]+P.d_label[2*k+1]){
                min = P.d_label[2*k]+P.d_label[2*k+1];
                meet =k;
            }
        }
    }
    *P.d_p_frontier_tail=meet;
}