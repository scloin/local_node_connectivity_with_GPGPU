...
/*
__global__ void matmul_with_CSR(){
    const int tid = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    *check0=-1;
    if(tid<NUM*NUM){
        do{            
            if (tid==0) {
                *check1=atomicExch(check0,0);
                atomicAdd(dd,1);
                }
            if(M<(edges[N+1]-edges[N])){
                k=input[dest[edges[N]+M]];
                if(output[N]!=-1){
                    atomicAdd(&output[N],k);}             
            }
            __syncthreads();
            if(tid<NUM){
                if(output[tid]==0) {
...
__global__ void BFSmain(int* in,int* out, int* edges, int*dest, int N){
    int pcheck=0;
    int ccheck=0;
    int num_blocks = (N*N+BLOCK_SIZE-1) / BLOCK_SIZE;
    int num_blocks1 = (N+BLOCK_SIZE-1) / BLOCK_SIZE;
    while(pcheck+ccheck<10){
        matmul<<<num_blocks,BLOCK_SIZE>>>(in,out, edges, dest, N);
        cudaDeviceSynchronize();
        filter<<<num_blocks,BLOCK_SIZE>>>(in,out, N);
        cudaDeviceSynchronize();
        ccheck++;
    }
}

__global__ void matmul(int* input, int* output, int* edges, int*dest, int NUM){
    int N = tid/NUM; int M = tid%NUM;
    if(M<(edges[N+1]-edges[N])){
        k=input[dest[edges[N]+M]];
        if(output[N]!=-1){
